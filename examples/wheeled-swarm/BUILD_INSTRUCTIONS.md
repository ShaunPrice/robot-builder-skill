...
- **Fiducials:** `python cad/make_tags.py 3` → `aruco_id0/1/2.png`. **Print at 100 % scale** so each
  marker is ~60 mm, cut out, and stick one flat in each chassis's top recess (id 0 on robot 0, etc.).

## 2. Wire it up
Common ground everywhere (battery −, DRV8833 GND, ESP32 GND, sensor GND).

**Motors — DRV8833 (2 inputs per motor):**

| ESP32 | → DRV8833 | DRV8833 out | → |
|---|---|---|---|
| GPIO2 | AIN1 | AO1 / AO2 | left motor |
| GPIO3 | AIN2 | | |
| GPIO4 | BIN1 | BO1 / BO2 | right motor |
| GPIO5 | BIN2 | | |
| 3V3 | STBY/EEP (enable) | VM | battery + (motor voltage) |

**Distance sensor — VL53L0X (I2C):** VCC→3V3, GND→GND, SDA→GPIO6, SCL→GPIO7. Mount it looking
**forward** through the chassis's front slot.

**Power:** the **4×AA pack** feeds the DRV8833 `VM` (motors). Power the ESP32 either from a small
**5 V buck** off the same pack (clean single-battery build) or from a **USB power bank** (easiest for
a first build) — either way, tie all grounds together. Pins are set at the top of `robot.ino` — edit
to match your board.

## 3. Flash the robots
- Arduino IDE with the **ESP32 core (3.x)**; install **Adafruit_VL53L0X** from Library Manager.
